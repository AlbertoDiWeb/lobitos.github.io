...
$query = "INSERT INTO cuentatutores (usuario, password) 
...
$max = "SELECT MAX(Id) as max_id FROM cuentatutores WHERE usuario = '$usuario'";     
$max1 =  mysqli_query($conn, $max);   
$row = mysqli_fetch_assoc($max1); 
$max_id=$row['max_id']; 
$insert = mysqli_query($conn, "INSERT INTO tutores(dni, nombre, primerApellido, segundoApellido, direccion, telefono, cuentaTutores_id) VALUES('$dni', '$nombre', '$primerApellido', '$segundoApellido', 
	'$direccion','$telefono','$max_id')") or die(mysqli_error());
if($insert){
...
<form name="form1" id="form1" class="form-horizontal row-fluid" action="registro.php" method="POST" >
	...
	<div class="control-group">
		...
		<div class="controls">
			<input name="telefono" id="telefono" class="form-control span8 tip" type="text" placeholder="Telefóno" required />
		</div>
	</div>
